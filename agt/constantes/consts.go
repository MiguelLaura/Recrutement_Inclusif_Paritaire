package constantes

const (
	POURCENTAGE_AGRESSEUR_F   = .02  // ...
	POURCENTAGE_AGRESSEUR_H   = .125 // ...
	GEN_F_H_CANDIDATS         = .5   // ...
	ANCIENNETE_MAX            = 40   // ...
	SANTE_MENTALE_MAX         = 100  // ...
	DEGATS_TRAUMATISME        = 20   // ...
	BOOST_TEAM_BUILDING       = 10   // Valeur peut-être à ajuster
	POURCENTAGE_FORMATION     = .32  // Pourcentage d'employés réalisant une formation par an
	PRIX_FORMATION            = 500  // Coût de la formation par jour et par employé
	NB_JOURS_FORMATION        = 7
	CONTRIBUTION_FORMATION    = .45 // Entreprise participe à 45% des coûts de formation
	COUNT_FORMATION           = 5   // Nombre de formations pour monter en compétences
	COMPETENCE_MIN            = 0
	COMPETENCE_MAX            = 10
	POURCENTAGE_RECRUTEMENT   = .05
	NB_CANDIDATS_PAR_OFFRE    = 18 // Source : https://blog.flatchr.io/barometre-des-entreprises-qui-recrutent-deuxieme-semestre-2022
	TIMEOUT_AGRESSION         = 4
	PROBA_PLAINTE             = .3
	PROBA_LICENCIEMENT        = .175
	POURCENTAGE_DEM_SPONTANEE = .027   // Taux de démission spontanée en France au 1er trimestre 2022
	RETENTION_PARITE          = .2     // Au sein d'un organisation inclusive, les employés ont une rétention supérieure de 20% (source: Catalyst)
	SEUIL_IMPACT_FEMME        = .35    // D'après Rosabeth Kanter, une minorité doit représenter au minimum 35% du groupe dominant pour avoir une influence
	CA_PAR_EMPLOYE            = 317000 // Source : https://xval.fr/chiffre-affaires-salarie/
	COUT_EMPLOYE              = 50850  // Source : https://www.legisocial.fr/actualites-sociales/269-savez-vous-combien-coute-un-salarie-en-france.html
	COUT_RECRUTEMENT          = 6500
	BOOST_PRODUCTIVITE_FEMME  = .2849 // Source : https://www.cairn.info/revue-travail-genre-et-societes-2010-1-page-181.htm
	SEUIL_AMENDE              = .4
	POURCENTAGE_AMENDE        = .01
)
